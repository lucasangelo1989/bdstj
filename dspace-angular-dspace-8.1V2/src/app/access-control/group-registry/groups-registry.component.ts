...
export class GroupsRegistryComponent implements OnInit, OnDestroy {
          ...
          switchMap((isSiteAdmin: boolean) => {
            return observableCombineLatest([...groups.page.map((group: Group) => {
              if (hasValue(group) && !this.deletedGroupsIds.includes(group.id)) {
                return observableCombineLatest([
                  ...
                  map(([canDelete, canManageGroup, hasLinkedDSO, subgroups, members]:
                    ...
                    groupDtoModel.epersons = members.payload;
                      // 🔹 Adicionando o console.log para verificar os valores
        		//console.log('Grupo:', groupDtoModel.group.name, ' - Usuários:', groupDtoModel.epersons?.totalElements || 0,  ' - Subgrupos:', groupDtoModel.subgroups?.totalElements || 0);
                ...
                dtos.sort((a, b) => (b.epersons?.totalElements || 0) - (a.epersons?.totalElements || 0));
  ...
  //customização pra resolver o problema de nome antigo
  logGroupName(groupDto: GroupDtoModel) {
  //console.log('Renderizando nome no HTML:', groupDto.group.name);
  return groupDto.group.name;
}
